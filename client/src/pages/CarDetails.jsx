  ...
  return car ? (
    <div className="px-6 md:px-16 lg:px-24 x1:px-32 mt-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {
                  ["360 Camera", "Bluetooth", "GPS", "Heated Seats", "Rear View Mirror"].map((item) => (
                    <li key={item} className="flex items-center text-gray-500">
                      <img src="{assets.check-icon}" className="h-4 mr-2" alt="" />
                      {item}
                    </li>
                  ))
                }
        ...
        {/* Right: Booking Form */}
        <form onSubmit={handleSubmit} className="shadow-lg h-max sticky top-18 rounded-x1 p-6 space-y-6 text-gray-500">
          ...
          <button className="w-full bg-primary hover:bg-primary-dull transition-all py-3 font-medium text-white rounded-x1 cursor-pointer">Book Now</button>
